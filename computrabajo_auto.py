# computrabajo_brave.py — Brave + Selenium (Computrabajo + IA integral, SOLO IA para texto con anti-genéricos)
# Flujo:
# 1) Abrir listado por palabra+ciudad.
# 2) Scrollear hasta ver "Siguiente", recolectar 20 links al DETALLE + href "Siguiente".
# 3) Recorrer 1..20: abrir detalle. Si hay preguntas (modal o embebidas) → IA responde (texto, radios, checkboxes, selects, números) y envía de forma robusta.
...
USE_LLM_FALLBACK = True                  # fallback IA (otra prompt) si falla lo principal
STRICT_IA_ONLY = False                   # si la IA falla, NO usar heurísticas (queda vacío)
    ...
    try:
        if LLM_BACKEND == "ollama":
            r = requests.post(f"{OLLAMA_URL}/api/generate",
                              ...
                              timeout=8)
            ok = r.ok and "response" in (r.json() or {})
            ia_log(f"LLM listo (Ollama / {LLM_MODEL}): {ok}")
            return ok
        elif LLM_BACKEND == "openai":
            if not OPENAI_API_KEY:
                ia_log("OpenAI sin API key.")
                return False
            headers = {"Authorization": f"Bearer {OPENAI_API_KEY}"}
            body = {"model": LLM_MODEL, "messages": [{"role":"user","content":"ok"}], "max_tokens": 2}
            r = requests.post(f"{OPENAI_BASE_URL}/chat/completions", json=body, headers=headers, timeout=8)
            ok = r.ok and "choices" in (r.json() or {})
            ia_log(f"LLM listo (OpenAI / {LLM_MODEL}): {ok}")
            return ok
        else:
...
# ==================== Anti-genéricos / hints de tema ====================
ANTI_GENERIC_PATTERNS = [
    r"\bs[ií],?\s*estoy disponible\b",
    r"\bs[ií],?\s*no tengo inconvenientes\b",
    r"\bpuedo adaptarme\b",
    r"\bcuento con la experiencia requerida\b",
    r"\ba convenir\b",
]
TOPIC_HINTS = {
    "iso 9001": "Si conoces ISO 9001, aclara nivel (básico/intermedio/avanzado) y un ejemplo breve de aplicación.",
    "itil": "Indica si aplicaste ITIL (incidentes, cambios, problemas) con una práctica concreta.",
    "sql": "Indica nivel (consultas, joins, SP) y entorno (MySQL/SQL Server) en una frase.",
    "redes": "Menciona experiencia concreta (TCP/IP, VLAN, cableado, mikrotik/cisco).",
    "soporte": "Indica experiencia en mesa de ayuda, tickets, resolución de incidencias y plataformas.",
}
def _has_generic(text: str) -> bool:
    if not text:
        return True
    lt = text.lower().strip()
    for pat in ANTI_GENERIC_PATTERNS:
        if re.search(pat, lt):
            return True
    return len(lt) < 12
def _topic_hint_from_question(q: str) -> str:
    ...
    for k, hint in TOPIC_HINTS.items():
        if k in ql:
            return hint
    if any(ql.startswith(x) for x in ("¿tenes", "tenes", "¿tienes", "tienes", "¿aplicaste", "aplicaste")):
...
def _llm_answer_per_question(question_label: str, cv: dict) -> str | None:
    """
    Devuelve SIEMPRE una frase breve y específica. Reintenta si la salida es genérica.
    """
    try:
        ...
        def call_llm(extra_rules: str) -> str | None:
            prompt = (
                ...
                f"{'Pista: ' + topic_hint if topic_hint else ''}\n\n"
                f"PREGUNTA: {q}\n"
                f"PERFIL: {perfil}\n"
                f"REGLAS EXTRA: {extra_rules}\n"
                "Devuelve SOLO la frase, sin comillas."
            )

            if not LLM_READY:
                ia_log("LLM no listo; _llm_answer_per_question -> None")
                return None

            ia_log(f"LLM texto ▶ Q='{q[:60]}{'…' if len(q)>60 else ''}'")
            if LLM_BACKEND == "ollama":
                r = requests.post(
                ...
                body = {
                    "model": LLM_MODEL,
                    "messages": [
                        {"role": "system", "content": "Responde en español, conciso, específico y profesional."},
        ...
        ia_log("LLM texto ✗ genérico/ vacío tras reintentos")
        return None
    except Exception as e:
        ia_log(f"LLM texto ✗ error: {e}")
        return None
...
def _llm_answer_for_fallback(question_label: str, cv: dict):
    try:
        ...
        if not LLM_READY:
            ia_log("LLM no listo (fallback); None.")
            return None
        if LLM_BACKEND == "ollama":
...
def _llm_choice_for_options(question_label: str, options_texts: list[str], cv: dict) -> str | None:
    try:
        ...
        if not LLM_READY:
            ia_log("LLM no listo (opciones); None.")
            return None
        ia_log(f"LLM opciones ▶ Q='{q[:50]}{'…' if len(q)>50 else ''}' | opciones={options_texts[:5]}{'…' if len(options_texts)>5 else ''}")
        if LLM_BACKEND == "ollama":
            resp = requests.post(f"{OLLAMA_URL}/api/generate",
                                 json={"model": LLM_MODEL, "prompt": prompt, "stream": False},
                                 timeout=20)
            ...
            headers = {"Authorization": f"Bearer {OPENAI_API_KEY}"}
            body = {"model": LLM_MODEL,
                    "messages": [{"role": "system", "content": "Elige UNA opción exacta; responde solo el texto."},
                                 {"role": "user", "content": prompt}],
                    "temperature": 0.0, "max_tokens": 20}
            resp = requests.post(f"{OPENAI_BASE_URL}/chat/completions", json=body, headers=headers, timeout=20)
            resp.raise_for_status(); text = (resp.json()["choices"][0]["message"]["content"] or "").strip()
        ia_log(f"LLM opciones ✓ '{text}'")
        return text.strip(" \n\"'") or None
    except Exception as e:
        ia_log(f"LLM opciones ✗ error: {e}")
        return None
...
def _llm_multi_choice_for_checkboxes(question_label: str, options_texts: list[str], cv: dict) -> list[str] | None:
    try:
        ...
        if not LLM_READY:
            ia_log("LLM no listo (checkboxes); None.")
            return None
        ia_log(f"LLM checkboxes ▶ Q='{q[:50]}{'…' if len(q)>50 else ''}' | opciones={options_texts[:5]}{'…' if len(options_texts)>5 else ''}")
        if LLM_BACKEND == "ollama":
            resp = requests.post(f"{OLLAMA_URL}/api/generate",
                                 json={"model": LLM_MODEL, "prompt": prompt, "stream": False},
                                 timeout=20)
            ...
            resp = requests.post(f"{OPENAI_BASE_URL}/chat/completions", json=body, headers=headers, timeout=20)
            resp.raise_for_status(); text = (resp.json()["choices"][0]["message"]["content"] or "").strip()
        if _norm(text) == "ninguna":
            ia_log("LLM checkboxes ✓ ninguna")
            return []
        parts = [p.strip(" \n\"'") for p in text.split(",") if p.strip()]
        ia_log(f"LLM checkboxes ✓ {parts}")
        return parts or None
    except Exception as e:
        ia_log(f"LLM checkboxes ✗ error: {e}")
        return None
...
def _llm_number_for_question(question_label: str, cv: dict, default: int = 2, min_v: int = 0, max_v: int = 50) -> int:
    try:
        ...
        if not LLM_READY:
            ia_log("LLM no listo (número); default.")
            return default
        ia_log(f"LLM número ▶ Q='{q[:60]}{'…' if len(q)>60 else ''}'")
        if LLM_BACKEND == "ollama":
            resp = requests.post(f"{OLLAMA_URL}/api/generate",
                                 json={"model": LLM_MODEL, "prompt": prompt, "stream": False},
                                 timeout=20)
        ...
        ia_log(f"LLM número ✓ {val}")
        return max(min_v, min(max_v, val))
    except Exception as e:
        ia_log(f"LLM número ✗ error: {e}")
        return default
    ...
    try:
        ...
        if pred:
            ...
            if 0 <= idx < len(options):
                ...
                ia_log(f"select ✓ '{texts[idx]}'")
                return True
        idx = _extract_positive_idx(texts)
        if 0 <= idx < len(options):
            sel.select_by_visible_text(options[idx].text)
            ia_log(f"select ✓ fallback '{texts[idx]}'")
            return True
        for opt in options:
            if (opt.text or "").strip():
                sel.select_by_visible_text(opt.text)
                ia_log(f"select ✓ primera no vacía '{opt.text}'")
                return True
    except: pass
    return False
...
def fill_questions_anywhere_and_submit(cv: dict) -> bool:
    ...
    for cont in containers:
        ...
        if radios and _pick_positive_radio(radios, question_label=label_ctx):
            ia_log(f"radio ✓ (ctx='{label_ctx[:40]}')")
            ok_algo = True

        if _check_all_checkboxes(cont, question_label=label_ctx):
            ia_log(f"checkbox ✓ (ctx='{label_ctx[:40]}')")
            ok_algo = True

        for s in cont.find_elements(By.TAG_NAME, "select"):
            if _select_positive_option(s, question_label=label_ctx):
                ok_algo = True

        if _fill_numeric_inputs(cont, label_ctx):
            ia_log(f"number ✓ (ctx='{label_ctx[:40]}')")
            ok_algo = True
            ...
            try:
                if ta.is_displayed() and (ta.get_attribute("value") or "").strip() == "":
                    ...
                    if ans != "":
                        ...
                        ia_log(f"textarea ← '{(ans[:50] + '…') if len(ans)>50 else ans}' (label='{qtxt[:40]}')")
            except: pass
